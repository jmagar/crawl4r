...
class QueueManager:
    ...
    async def acquire_lock(self, owner: str) -> bool:
        ...
        """
        result = await self._client.set(LOCK_KEY, owner, nx=True, ex=LOCK_TTL_SECONDS)
        return bool(result)
    ...
    async def set_status(self, info: CrawlStatusInfo) -> None:
        ...
        await self._client.lpush(RECENT_LIST_KEY, info.crawl_id)
        await self._client.ltrim(RECENT_LIST_KEY, 0, 49)
    ...
    async def get_status(self, crawl_id: str) -> CrawlStatusInfo | None:
        ...
        """
        raw = await self._client.get(f"{STATUS_PREFIX}{crawl_id}")
        if raw is None:
            return None
    ...
    async def list_recent(self, limit: int = 10) -> list[CrawlStatusInfo]:
        ...
        """
        crawl_ids = await self._client.lrange(RECENT_LIST_KEY, 0, limit - 1)
        statuses: list[CrawlStatusInfo] = []
        for raw_id in crawl_ids:
            crawl_id = raw_id.decode()
