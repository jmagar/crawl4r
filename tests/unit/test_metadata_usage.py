...
def _count_runtime_usages(source: str, key: str) -> int:
    ...
    typed_dict_indent: int | None = None
    for line in source.split("\n"):
        stripped = line.strip()
        if typed_dict_indent is not None:
            current_indent = len(line) - len(line.lstrip())
            if stripped == "" or current_indent <= typed_dict_indent:
                typed_dict_indent = None
            else:
                continue
        if "TypedDict" in line and (
            stripped.startswith("class ") or "TypedDict(" in line
        ):
            typed_dict_indent = len(line) - len(line.lstrip())
            continue
...
def test_qdrant_imports_metadata_keys():
    ...
    source = (PROJECT_ROOT / "crawl4r" / "storage" / "qdrant.py").read_text()
...
def test_processor_imports_metadata_keys():
    ...
    source = (PROJECT_ROOT / "crawl4r" / "processing" / "processor.py").read_text()
    assert "from crawl4r.core.metadata import MetadataKeys" in source, (
...
def test_recovery_imports_metadata_keys():
    ...
    source = (PROJECT_ROOT / "crawl4r" / "resilience" / "recovery.py").read_text()
    assert "from crawl4r.core.metadata import MetadataKeys" in source, (
...
def test_failed_docs_no_longer_imports_metadata_keys():
    ...
    source = (PROJECT_ROOT / "crawl4r" / "resilience" / "failed_docs.py").read_text()
