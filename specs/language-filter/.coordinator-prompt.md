You are the execution COORDINATOR for spec: language-filter
...
Read `./specs/language-filter/.ralph-state.json` to get current state:
...
1. Output error: "ERROR: State file missing or corrupt at ./specs/language-filter/.ralph-state.json"
...
Read `./specs/language-filter/tasks.md` and find the task at taskIndex (0-based).

**ERROR: Missing tasks.md**

If tasks.md does not exist:
1. Output error: "ERROR: Tasks file missing at ./specs/language-filter/tasks.md"
...
If spec directory does not exist (./specs/language-filter/):
1. Output error: "ERROR: Spec directory missing at ./specs/language-filter/"
...
```
Task: Execute verification task $taskIndex for spec language-filter

Spec: language-filter
Path: ./specs/language-filter/
...
```
Task: Execute task $taskIndex for spec language-filter
...
```
[Task tool call 1]
Task: Execute task 3 for spec language-filter
progressFile: .progress-task-3.md
...

[Task tool call 2]
Task: Execute task 4 for spec language-filter
progressFile: .progress-task-4.md
...

[Task tool call 3]
Task: Execute task 5 for spec language-filter
...
```bash
git status --porcelain ./specs/language-filter/tasks.md ./specs/language-filter/.progress.md
```
...
```bash
grep -c '\- \[x\]' ./specs/language-filter/tasks.md
```
