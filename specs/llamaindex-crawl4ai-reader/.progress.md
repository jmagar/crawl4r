...
- [x] 2.6.3b Verify tests still pass after refactor - verified
- [x] V5 Quality checkpoint: single URL crawling complete - (current commit)
- [x] 2.7.1a RED: Test for deduplication enabled - 589b4e5
- [x] 2.7.1b RED: Test for deduplication disabled - d6d0106
- [x] 2.7.1c RED: Test for no vector_store (deduplication skipped) - beaffef
- [x] 2.7.2a GREEN: Add enable_deduplication and vector_store fields - 8a24a7b
- [x] 2.7.2b GREEN: Implement _deduplicate_url method - 677fc0c
- [x] 2.7.2c GREEN: Call _deduplicate_url in aload_data - 80290f2
- [x] 2.7.3a REFACTOR: Document _deduplicate_url (already complete from 2.7.2b)
- [x] 2.7.3b REFACTOR: Verify deduplication tests still pass - verified
- [x] 2.8.1a Write test for empty URL list - (already GREEN)
- [x] 2.8.1b Write test for single URL - 4ba0a02
- [x] 2.8.1c Write test for multiple URLs concurrent processing - 1f22454
- [x] 2.8.1d Write test for order preservation with failures - 4f2e0a6
- [x] 2.8.1e Write test for concurrency limit enforcement - df58a2b
- [x] 2.8.1f Write test for batch statistics logging - 297e836
- [x] 2.8.2a Acknowledge aload_data already implemented in task 2.7.2c - a522d99
- [x] 2.8.3a REFACTOR: Acknowledge crawl_with_semaphore wrapper extracted in 2.7.2c - 538fcbc
- [x] 2.8.3b REFACTOR: Acknowledge async context warning in docstring from 2.7.2c - 538fcbc
- [x] 2.8.3c REFACTOR: Verify aload_data tests still pass - 538fcbc
- [x] 2.9.1a RED: Write test for load_data delegates to aload_data - (pending commit)
- [x] 2.9.1b RED: Write test for load_data with single URL - (pending commit)
- [x] 2.9.2a GREEN: Implement load_data method - 4aded1a
- [x] 2.9.3a REFACTOR: Add load_data docstring (already complete from 2.9.2a)
- [x] 2.9.3b REFACTOR: Verify tests still pass after refactor - verified
- [x] V6 Quality checkpoint: full reader implementation - d8ab7f1
...
- [x] 2.10.3a Enhance error messages with URL context - c29c344
- [x] 2.10.3b Verify tests still pass after refactor - 80af04b
- [x] V7 Quality checkpoint: Phase 2 complete - e021317
- [x] 3.1.1 Write integration test for health check - 6ddf342
- [x] 3.1.2 Write integration test for single URL crawl - 0fc435e (with API fix)
- [x] 3.1.3 Write integration test for batch crawl - f5e396e
- [x] 3.1.4 Write integration test for circuit breaker - fb0e418
- [x] 3.1.5 Write integration test for concurrency - 2c65892
- [x] V8 Integration tests pass - (verification only, no commit needed)
- [x] 3.2.1 Create E2E test file - 06d1ae5
- [x] 3.2.2 Write E2E test for reader to chunker - 2235179
- [x] 3.2.3 Write E2E test for reader to Qdrant - 9ae0c71
- [x] V9 E2E tests pass - (verification only, no commit needed)

## Current Task

Task 3.3.1: Update CLAUDE.md with reader usage

## Learnings

### Phase 3.2 E2E Pipeline Testing Learnings

- E2E test file structure mirrors integration test patterns: fixtures for all components (reader, chunker, TEI, vector store)
- TEIClient constructor takes individual parameters (endpoint_url, dimensions, timeout, max_retries), not Settings object
- MarkdownChunker.chunk() takes text and filename parameters, returns list[ChunkDict] with chunk_text field (not text field)
- ChunkDict structure includes: chunk_text, chunk_index, section_path, heading_level fields
- E2E tests should include graceful skipping when services unavailable (pytest.skip with explanatory message)
- test_e2e_reader_to_chunker verifies reader crawls URL and chunker produces valid chunk structure
- test_e2e_reader_to_qdrant verifies full pipeline: crawl → chunk → embed → store → query → cleanup
- Full pipeline test validates source_url field in Qdrant metadata (Issue #17 verification)
- E2E tests use actual service URLs from environment variables (TEI_ENDPOINT, QDRANT_URL, CRAWL4AI_URL)
- Test cleanup important: delete_by_url() after Qdrant tests to avoid polluting test collection
- VectorMetadata requires file_path_relative and file_path_absolute (use URL for both in web crawl case)
- Qdrant scroll API with Filter and FieldCondition enables querying by source_url field
- E2E test validation: 1 test passes with Crawl4AI service, 1 test skips gracefully without TEI/Qdrant
- Phase 3.2 complete: 2 E2E tests written, V9 checkpoint passed

### Phase 3 Integration Testing Learnings

- Crawl4AI v0.5.1 API differs from design: expects `{"urls": [...]}` not `{"url": "..."}`
- API response format: `{"success": true, "results": [{crawl_result}, ...]}` (batch response even for single URL)
- Integration test with real Crawl4AI service (example.com) passes after API fix
- Implementation updated: POST payload changed from `{"url": url, "crawler_params": {...}}` to `{"urls": [url]}`
- Response parsing updated: Extract `results[0]` from batch response for single URL requests
- Batch integration test verifies multiple URLs crawled concurrently with order preservation
- Test URLs (example.com, example.org, example.net) are reliable for integration testing
- Integration tests complete in ~1.5s for 3 URLs demonstrating good concurrent performance
- Circuit breaker integration test verifies failures trigger OPEN state after threshold
- Circuit breaker state property returns string "OPEN" not CircuitState.OPEN enum
- Circuit breaker internal state is _state (enum), exposed via state property (string)
- Invalid domain URLs reliably trigger 500 errors from Crawl4AI service (good for testing)
- Concurrency integration test with 10 URLs completes in ~12s (demonstrates parallel processing)
- Concurrency limit of 3 provides good balance between speed and resource usage
- Test validates semaphore-based concurrency control works with real service
- Unit tests need updating: 15 tests failing due to mock response format mismatch
- Created `wrap_api_response()` helper to wrap crawl results in proper API format for mocks
- `@pytest.fixture` decorator required for `mock_crawl_result_success()` when used as test parameter
- sed command applied to bulk-update response mocks (successful for simple cases)

## Blockers

**CRITICAL: Unit test mocks need systematic update**
- 15/44 unit tests failing after API format change
- All failing tests use old mock format: `httpx.Response(200, json=mock_response)`
- Need to update to: `httpx.Response(200, json=wrap_api_response([mock_response]))`
- Affects tests: crawl_single_url variants, retry tests, deduplication tests, aload_data tests
- Integration tests (2/2) passing with real service
- Recommendation: Batch update all remaining mocks systematically

## Additional Learnings (General)
...
## Additional Learnings (Task 2.10 Error Handling)

- Error handling tests verify existing implementation behavior (GREEN immediately)
- Test structure for error handling: mock exception, verify retry logic, check fail_on_error behavior
- timeout/network errors are transient (retry with backoff), JSON errors are permanent (no retry)
- fail_on_error default is False (graceful failures), must set True explicitly for exception propagation
- Error messages should include URL context for debugging (enhanced ValueError for no markdown)
- Settings integration was implemented but test needed health check mock for custom URL
- Reader __init__ now accepts optional settings parameter, uses Settings.CRAWL4AI_BASE_URL if provided
- Phase 2 complete: 44 unit tests, 92.62% coverage (exceeds 85% target)
...
Fix unit test mocks (15 failing tests) OR continue with Task 3.1.3 (batch crawl integration test)
...
## Additional Learnings (Task V5)

- Quality checkpoint after single URL crawling implementation found 3 line-length violations (>88 chars)
- Ruff enforces 88-character line limit on both code lines and f-string expressions
- Fixed by breaking long f-strings across multiple lines and moving extra dict to multiple lines
- Both ruff check and ty check now pass with zero errors
- Line breaks in f-strings use implicit string concatenation: f"part1 " f"part2"
- Structured logging extra dict should be formatted with one field per line for readability and line length compliance

## Additional Learnings (Task 2.7.1a)

- Deduplication test structure: mock VectorStoreManager with AsyncMock for delete_by_url method
- unittest.mock.MagicMock creates mock object, AsyncMock wraps async method for proper await support
- Test verifies delete_by_url called for each URL before crawling (call_count and assert_any_call)
- Mock pattern: mock_vector_store.delete_by_url = AsyncMock(return_value=5) for async method
- RED phase test fails with NotImplementedError: aload_data falls back to base class lazy_load_data
- Pydantic allows extra fields by default, so enable_deduplication and vector_store parameters accepted without validation error
- Test properly fails because aload_data method doesn't exist yet (correct RED phase failure)

## Additional Learnings (Task 2.7.1b)

- Deduplication disabled test mirrors enabled test but asserts delete_by_url NOT called (call_count == 0)
- Test sets enable_deduplication=False in reader constructor to verify conditional behavior
- Mock object still created and passed but should remain unused when deduplication disabled
- Test structure identical to 2.7.1a but with opposite assertion: verify method NOT called vs called
- RED phase test fails with same NotImplementedError as 2.7.1a (aload_data not implemented yet)
- Proper TDD for boolean flags requires testing both True and False paths separately

## Additional Learnings (Task 2.7.1c)

- No vector_store test verifies graceful handling when vector_store=None (even if enable_deduplication=True)
- Test sets vector_store=None explicitly to simulate missing vector store instance
- Edge case: user may enable deduplication but forget to provide vector_store - should not crash
- Test asserts documents returned successfully without deduplication errors
- Test verifies document content is correct (not just that it didn't crash)
- RED phase test fails with same NotImplementedError as 2.7.1a/b (aload_data not implemented yet)
- Proper edge case testing requires testing None/missing values for optional dependencies

## Additional Learnings (Task 2.7.2a)

- Added enable_deduplication (bool, default True) and vector_store (VectorStoreManager | None, default None) fields to Crawl4AIReader class
- Import VectorStoreManager from rag_ingestion.vector_store module required for type annotation
- Field placement after retry_delays and before is_remote property maintains logical grouping (config fields together)
- Field descriptions match design.md spec exactly: deduplication prevents duplicates, vector_store is optional dependency
- Pydantic Field() provides clear documentation for both fields visible in type hints and docs
- Modern Python 3.10+ union syntax: VectorStoreManager | None instead of Optional[VectorStoreManager]
- Verification confirms 5 matches (2 field declarations + 3 description strings)

## Additional Learnings (Task 2.7.2b)

- Implemented _deduplicate_url async method with early return pattern when vector_store is None
- Method calls await self.vector_store.delete_by_url(url) to remove all chunks for URL before re-crawling
- Structured logging includes both URL and deleted_count in extra dict for observability
- Comprehensive Google-style docstring explains behavior, args, returns, examples, and notes
- Method placement after _build_metadata and before _crawl_single_url maintains logical flow
- Deduplication pattern matches file watcher's delete-then-process behavior for consistency
- Early return enables graceful handling when vector_store not configured (no crash, no error)
- Log message format: "Deleted {deleted_count} old vectors for {url}" with structured extra fields

## Additional Learnings (Task 2.7.2c)

- Implemented complete aload_data method with async health check, deduplication, and concurrency control
- Health check validates Crawl4AI service before batch processing: await self._validate_health()
- Deduplication loop runs before crawling if enable_deduplication=True and vector_store is not None
- Concurrency control uses asyncio.Semaphore(self.max_concurrent_requests) to limit parallel requests
- Batch crawling uses asyncio.gather with return_exceptions=not self.fail_on_error for order preservation
- Order preservation critical: results list matches input URLs list length with None for failures
- Shared httpx.AsyncClient for connection pooling across all URLs in batch
- Nested crawl_with_semaphore function enforces semaphore limit on each URL crawl
- Batch statistics logging includes total, succeeded, and failed counts for observability
- Pydantic vector_store field required SkipValidation annotation to allow MagicMock in tests
- Import additions: typing_extensions.Annotated and pydantic.functional_validators.SkipValidation
- ConfigDict(arbitrary_types_allowed=True) enables VectorStoreManager type in Pydantic model
- All 3 deduplication tests pass after implementation: enabled, disabled, no vector_store

## Additional Learnings (Task 2.7.3a)

- _deduplicate_url docstring was already comprehensive from task 2.7.2b (lines 385-435)
- Docstring includes complete Args, Returns, Examples, and Notes sections per Google style
- Notes section explicitly references file watcher pattern alignment (lines 389, 426)
- grep verification confirms "file watcher" references present in docstring
- Task completed without code changes since docstring already met all requirements
- GREEN phase implementation (2.7.2b) included forward-looking documentation

## Additional Learnings (Task 2.7.3b)

- REFACTOR verification checkpoint: All 3 deduplication tests still pass after documentation (100% pass rate)
- Post-refactor test run confirms no regressions from earlier REFACTOR tasks
- Test categories verified: deduplication enabled, disabled, no vector_store edge case
- Comprehensive deduplication feature complete: fields, method, integration, documentation, tests
- Ready to proceed to next feature (async batch loading implementation)

## Additional Learnings (Task 2.8.1a)

- Empty URL list test written for aload_data method edge case handling
- Test immediately PASSES (GREEN) because aload_data was already implemented in task 2.7.2c with empty list handling (lines 664-665)
- Early return pattern (if not urls: return []) optimizes empty input without health check or network operations
- Test verifies: empty list returns empty list, no HTTP requests made, no service validation performed
- Edge case handling already complete from batch loading implementation
- Test documents expected behavior: AC-3.1 (edge case handling), FR-14 (batch loading with no URLs)

## Additional Learnings (Task 2.8.1b)

- Single URL batch test written for aload_data method with one URL
- Test immediately PASSES (GREEN) because aload_data was already fully implemented in task 2.7.2c
- Test verifies: health check called, single URL crawled, list with one Document returned, Document has correct content
- Test structure: mock health check, mock crawl response, call aload_data([url]), verify single Document in list
- Verifies AC-2.1, AC-2.2 (single URL batch processing), FR-14 (async batch loading)
- aload_data implementation from 2.7.2c handles all batch sizes: empty, single, multiple URLs

## Additional Learnings (Task 2.8.1c)

- Multiple URLs concurrent test written for aload_data method with 3 URLs
- Test immediately PASSES (GREEN) because aload_data was already fully implemented in task 2.7.2c with concurrent processing
- Test verifies: health check called, all URLs crawled, list with all Documents returned in same order, all Documents have correct content
- respx mocking challenge: Multiple mock.return_value() calls overwrite each other - must use side_effect callback instead
- side_effect pattern: Read request body JSON, extract URL, return appropriate mock response for that URL
- Test structure: mock health check, mock crawl responses with side_effect, call aload_data(urls), verify all Documents in correct order
- Order preservation verification: Assert documents[0], documents[1], documents[2] match page1, page2, page3 respectively
- Verifies AC-3.1 (multiple URLs processing), AC-3.2 (concurrent requests), AC-3.3 (concurrency control), US-3 (batch crawling)

## Additional Learnings (Task 2.8.1d)

- Order preservation test verifies [success, failure, success] → [Document, None, Document] pattern
- Test immediately PASSES (GREEN) because aload_data (task 2.7.2c) already implements order preservation
- Order preservation achieved via return_exceptions parameter in asyncio.gather (not return_exceptions=True)
- Test structure: fail_on_error=False (graceful failures), side_effect returns success=False for middle URL
- Three assertions verify order: documents[0] is not None, documents[1] is None, documents[2] is not None
- Critical for callers: enables correlation between input URLs and result Documents by index
- None in results list indicates failure at that position (not filtered out)
- Test verifies AC-3.4 (order preservation with failures), Issue #1 (results maintain input order)
- Pattern alignment: matches existing aload_data implementation using gather with return_exceptions

## Additional Learnings (Task 2.8.2a)

- Task 2.8.2a is acknowledgment task: aload_data was fully implemented in task 2.7.2c (commit 80290f2)
- All 6 aload_data tests pass immediately: empty_list, single_url, multiple_urls, order_preservation, concurrent_limit, logging
- Implementation includes all required features: health check, deduplication loop, semaphore control, AsyncClient pooling, gather with return_exceptions, statistics logging
- GREEN phase implementation was comprehensive in task 2.7.2c, no additional code needed
- Task 2.8.2a confirms implementation completeness before proceeding to REFACTOR phase

## Additional Learnings (Task 2.8.3a-c)

- Task 2.8.3a: crawl_with_semaphore wrapper already extracted in task 2.7.2c (lines 692-695)
- Wrapper function enforces concurrency limit via semaphore: async with semaphore wraps _crawl_single_url call
- Task 2.8.3b: Warning section already exists in aload_data docstring (lines 615-616) with async context requirement
- Warning message clear and actionable: "Must be called from async context. For synchronous code, use load_data()."
- Task 2.8.3c: REFACTOR verification confirms all 6 aload_data tests still pass (100% pass rate)
- REFACTOR phase (2.8.3) was actually completed during GREEN phase (2.7.2c) implementation
- Forward-looking GREEN implementations can include REFACTOR-quality code organization from the start
- All three REFACTOR tasks (2.8.3a-c) are acknowledgment tasks with no code changes needed

## Additional Learnings (Task 2.9.1a-b)

- Task 2.9.1a: test_load_data_delegates_to_aload_data verifies load_data calls aload_data and returns result
- Task 2.9.1b: test_load_data_single_url verifies synchronous API works end-to-end with real mock responses
- Both tests properly FAIL with NotImplementedError: Crawl4AIReader does not provide lazy_load_data method
- Base class (BasePydanticReader) load_data calls lazy_load_data which needs override or direct implementation
- Test structure uses respx.mock decorator with inline imports following project pattern
- Mock pattern: AsyncMock for aload_data with patch.object to verify delegation behavior
- RED phase tests confirm load_data method needs implementation to override base class behavior

## Additional Learnings (Task 2.9.2a)

- Implemented load_data method as simple wrapper: return asyncio.run(self.aload_data(urls))
- Method includes comprehensive Google-style docstring with Args, Returns, Raises, Examples, Notes sections
- Warning section alerts users that method blocks, recommending aload_data() for async contexts
- Examples demonstrate both basic usage and graceful failure handling (fail_on_error=False)
- Notes section clarifies asyncio.run() wrapping pattern and behavior parity with aload_data
- Test fix required: patch.object on Pydantic class methods requires patching at class level, not instance
- Test assertion pattern: when patching at class level, self is not included in mock call args
- Both load_data tests now PASS: delegation test verifies aload_data called, functional test verifies end-to-end behavior
- type: ignore[override] comment suppresses mypy complaint about return type annotation (Document | None vs Document)
- GREEN phase complete: synchronous API implemented with comprehensive documentation

## Additional Learnings (Task 2.9.3a-b)

- Task 2.9.3a: Docstring already complete from GREEN phase (2.9.2a), no additional work needed
- Forward-looking GREEN implementations can include comprehensive documentation from the start
- REFACTOR verification (2.9.3b): All 8 load_data-related tests still pass (100% pass rate)
- Test categories: aload_data (6 tests), load_data delegation (1 test), load_data functional (1 test)
- Synchronous loading feature complete: implementation, documentation, tests all passing

## Additional Learnings (Task V6)

- Quality checkpoint V6 found line-length violations in docstrings (>88 chars)
- Ruff enforces 88-character line limit on all code including docstrings and comments
- Fixed by breaking long lines in Raises and Notes sections of load_data docstring
- Fixed test file line-length issues: shortened docstrings, moved inline comments, split list comprehensions
- Both ruff check and ty check pass with zero errors on crawl4ai_reader.py
- Full reader implementation complete with all quality gates passed
- Ready to proceed to error handling tests (Phase 2.10)
